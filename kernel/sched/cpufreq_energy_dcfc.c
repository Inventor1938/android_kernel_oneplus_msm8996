...
static void nrggov_fast_switch(struct cpufreq_policy *policy,
			      unsigned int next_freq)
{
				...
				int cpu, bool remote, unsigned int next_freq)
{
	...
	if (policy->fast_switch_enabled && !remote) {
		...
		nrggov_fast_switch(policy, next_freq);
				...
				unsigned int flags)
{
	...
	/* Skip updates generated by nrggov kthreads */
	...
	if (nrggov_should_update_freq(sg_policy, time)) {
		next_f = nrggov_next_freq_shared(sg_cpu);
		nrggov_update_commit(sg_policy, time, cpu, remote, next_f);
	}
...
{
	struct nrggov_policy *sg_policy = container_of(irq_work, struct
						      nrggov_policy, irq_work);
	struct cpufreq_policy *policy = sg_policy->policy;

	if (policy->fast_switch_enabled) {
		nrggov_fast_switch(policy, sg_policy->next_freq);
		sg_policy->work_in_progress = false;
		return;
	}

	/*
	 * For RT and deadline tasks, the energy-dcfc governor shoots the
	 * frequency to maximum. Special care must be taken to ensure that this
	 * kthread doesn't result in the same behavior.
	 *
	 * This is (mostly) guaranteed by the work_in_progress flag. The flag is
	 * updated only at the end of the nrggov_work() function and before that
	 * the energy-dcfc governor rejects all other frequency scaling requests.
	 *
...
static
#endif
struct cpufreq_governor energy_dcfc_gov = {
	.name = "energy-dcfc",
